...
### `post/doing/fish-oil-tools.html`
- H1: 魚油智慧工具箱
- H2: 純度與價值分析儀
- H2: 個人化魚油挑選器
- H2: 超級比一比
...
- [x] **輸入**: 
  ...
  - 核心需求 (護眼/護心/幫助入睡/其他)
  - 體重 (kg)
- [x] **邏輯**: 參照 WHO/GOED 建議標準，動態生成建議。
  - **孕婦**: 優先推薦 DHA 高佔比，強調 300mg DHA。
  - **銀髮**: 優先推薦純 EPA 或 EPA:DHA > 2:1。
- [x] **輸出**: 「你的每日建議處方」卡片 (建議總劑量 mg + 膠囊數)。

### D3. 魚油 vs 藻油 vs 磷蝦油 終極比較表 (Interactive Comparison)
> *SEO 目標: 鎖定「素食魚油」、「磷蝦油比較」*

- [x] **功能**: 互動式開關 (Switch)，動態顯示比較。
  - [x] 素食者 -> 顯示「裂壺藻油」
  - [x] 重視吸收 -> 顯示「磷蝦油 (磷脂質)」 vs 「rTG 魚油」
- [x] **呈現**: 橫向捲動比較表 (Component-based)。 

### D4. 結果保存與分享 (Viral Loop)
- [x] **功能**: "Generate My Prescription" 按鈕。
- [x] **實作**: 使用 HTML5 Canvas 生成包含計算結果的圖片 (JPG/PNG)。
- [x] **導流**: 圖片上需壓印網站 Logo 與 QR Code 回到工具頁。
...
## 5. 驗收標準
- [x] 計算結果準確無誤 (需提供 3 組測試案例)。
- [x] 手機版輸入體驗良好 (Input type="number")。
- [x] 圖片生成功能在手機瀏覽器 (Safari/Chrome) 可正常運作。
- [x] Core Web Vitals (LCP/CLS) 不受工具載入影響。
