...
### B3. 深度知識模組 (Deep Dive)
- [ ] **功效段落**: 檢查並補上 PubMed/JAMA 參考文獻連結。
- [ ] **副作用段落**: 誠實揭露 (如：拉肚子、凝血功能影響)，增加可信度。
- [ ] **吃法段落**: 明確建議 (隨餐吃)，並解释脂溶性原理。

### B4. 常見問答 (FAQ Module)
- [ ] **內容**: 針對 Feature Snippet 設計的 Q&A。
- [ ] **技術**: 注入 JSON-LD FAQPage Schema 到 `<head>`。
- [ ] **互動**: 使用 `<details>` 與 `<summary>` 標籤實作摺疊效果。

## 4. 驗收標準
- [ ] TL;DR 區塊在手機版不佔據過多首屏空間。
- [ ] FAQ 結構化數據通過 Google Rich Results Test。
- [ ] 所有引用連結有效。
